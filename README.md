...
给 Xiaomi HyperOS 的音乐闹钟。银白、青绿与圆角卡片，搭配银发青瞳角色，让早晨从喜欢的旋律开始。

**v1.2.0：** 在 v1.1.0 基础上，设定闹钟只保留「保存闹钟」入口，切模块／返回不再弹保存确认，只保留草稿；网易云选定的歌曲会显示具体曲目；首页问候、响铃文案和角色状态随手机本地时间变化。
...
在本仓库 [Releases](../../releases/latest) 下载 `Qingling-1.2.0.apk`，传到手机安装。`aab` 是用于分发平台的 Android App Bundle，不能直接点击安装。`SHA256SUMS.txt` 可用于校验下载文件。升级可覆盖安装，保留闹钟和已导入音乐。
...
桌面组件与权限模块：[闹钟表](docs/screenshots/widget.png) · [系统设置](docs/screenshots/settings.png) · [选曲检查](docs/screenshots/song-picker.png)。截图来自 Android 15 模拟器。

| 功能 | 行为 |
| --- | --- |
| 响铃方式 | 纯音乐、纯振动、音乐 + 振动 |
| 耳机播放 | `USAGE_MEDIA`，显式优先已连接的蓝牙 A2DP／LE 或有线媒体耳机；导入音乐由 MediaCodec 解码为 PCM 立体声，通过 AudioTrack 播放，避免压缩音频卸载和空间音频处理 |
| 耳机断开 | 收到系统音频路由断开广播后停止音乐、改为振动，避免意外外放 |
| 本地导入 | 系统文件选择器，复制到应用内部存储并验证音频；支持设备可解码的 MP3、M4A、FLAC、WAV 等，单文件不超过 128 MiB |
| 网易云选歌 | 点击跳转网易云，从歌单选择指定歌曲后“分享 → 更多 → 青铃”；也支持粘贴歌曲／歌单链接。确认页提前检查可导入状态，受限曲目标红、禁止勾选，选择 1–20 首缓存，选定后显示具体曲目 |
...
| 桌面小组件 | 固定 4×4 的完整闹钟表，上方显示下一次响铃，下方 ListView 可滚动查看全部开启／关闭闹钟；点击某行编辑；已添加时按钮禁用，配置入口拒绝重复添加 |
| 编辑保存 | 只保留「保存闹钟」；返回或切换模块不弹确认，只保留草稿，点击保存后才更新实际闹钟 |
| 时间文案 | 首页问候、响铃页提示和角色状态按本地时间在凌晨／早晨／午间／下午／晚间切换 |
| 权限状态 | 系统可检测的缺失权限标红并提供跳转；HyperOS 自启动、后台弹窗等无公开检测接口的项目标为待手动确认，不伪报已授权 |
...
从 v1.0 升级时，如果桌面已有旧版的小组件，请先移除旧组件，再添加一次新版闹钟表；桌面宿主不会自动扩大旧组件占用的格子。移除组件不删除闹钟。固定格子尺寸由 Android Launcher / HyperOS 桌面解释。
...
网易云没有向本应用提供“打开歌单选择器并返回所选歌曲”的授权 API，因此采用**跳转网易云选歌 → 用户分享回青铃 → 在青铃检查并确认导入**。青铃不能在网易云原生界面中注入可导入标签，标红发生在青铃自己的确认页。支持部分官方短链接跳转解析；无法解析时复制完整的 `music.163.com/song?id=…` 或歌单链接。

缓存后不依赖网易云在后台运行。私有歌单、NCM 加密文件不支持，歌单不会自动同步；公开接口变动时可改用本地导入。
...
v1.1.0：16 项 JVM 测试通过；Android 15 / API 35 模拟器 29 项设备检查通过，包含本地与网易云压缩音频 PCM 播放、可导入状态探测、完整桌面列表数据、模块切换与返回保存。设备 UI 检查需先解锁测试模拟器；使用 `-e network true` 启用网络检查。

用户已在 Xiaomi 真机与 Baseus Bowie H1i 蓝牙耳机上确认内置和导入音乐均有声音。完整的长期待机、所有 HyperOS 权限组合和不同耳机型号仍需真机验收，不能据此保证系统永不回收进程。

HyperOS 真机验收清单见 [docs/DEVICE-TEST.md](docs/DEVICE-TEST.md)。

## 设计与隐私

v1.1.0 按用户指定使用 [Whom001x/-](https://github.com/Whom001x/-) 的角色图片，去除洋红背景后用于界面和头像图标，详见 [素材来源](docs/ARTWORK.md)。内置铃声仍为本项目原创合成音乐。
